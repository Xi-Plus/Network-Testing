...
$point_size=5;
foreach($row as $temp){
	imagefilledellipse($img,P(strtotime($temp["time"]),$start_time,$end_time,$start_x,$end_x),P($temp["percent"],0,100,$end_y,$start_y),$point_size,$point_size,imagecolorallocate($img,$config["domain"][$temp["domain"]][0],$config["domain"][$temp["domain"]][1],$config["domain"][$temp["domain"]][2]));
}
...
?>
<font color="#<?php foreach($color as $temp){echo str_pad(dechex($temp),2,"0",STR_PAD_LEFT);}; ?>"><?php echo $domain; ?></font><br>
<?php
}
?>
<br>
